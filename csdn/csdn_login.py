...
def get_post_data():
    ...
    login_page = session.get(url, headers=get_post_headers(), cookies=cookies).text

    time.sleep(5) # 停5秒钟，就能成功，why???

    # 提取lt, execution, _eventId
    soup = BeautifulSoup(login_page, 'html.parser')
    lt = soup.find(attrs={'name': 'lt'})['value']
    execution = soup.find(attrs={'name': 'execution'})['value']
    # fkid = soup.find(attrs={'name': 'fkid'})['value']  # 增加一个新的字段
...
# 返回登录过后的session
def login(post_data):
    session.post(url, data=post_data, headers=get_post_headers(), cookies=cookies)
    return session
...
if __name__ == '__main__':
    ...
    resp = session.get(home_page, headers=headers)
    print(resp.text)
